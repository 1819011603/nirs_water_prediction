...
class SPA:
    ...
    def _validation(self, Xcal, ycal, var_sel, Xval=None, yval=None):
        ...
        # 使用单独的验证集进行验证
        if NV > 0:
            Y_cal_class = pd.get_dummies(ycal)
            plsda = PLSRegression(n_components=2)
            try:
                plsda.fit(Xcal[:, var_sel.astype(np.int)], Y_cal_class)
                Y_val_hat = plsda.predict(Xval[:, var_sel.astype(np.int)])
                yhat = np.array([np.argmax(i) for i in Y_val_hat])
            except:
                print("Invalid number of components",var_sel)
            ...
            # 为yhat 设置适当大小
            yhat = []
            Y_class = pd.get_dummies(ycal)
            loo = LeaveOneOut()
            for train, test in loo.split(Xcal):
                plsda = PLSRegression(n_components=6)
                plsda.fit(Xcal[train, var_sel.astype(np.int)], Y_class[train])
                yhat.append(plsda.predict(Xcal[test, var_sel.astype(np.int)]))
    ...
    def spa(self, Xcal, ycal, m_min=1, m_max=None, Xval=None, yval=None, autoscaling=1):
        ...
        plt.ylabel('RMSE')
        print(len(var_sel),i_crit,len(RMSEP_scree))
...
if __name__ == "__main__":
    ...
    var_sel, var_sel_phase2 = SPA().spa(
        Xcal, ycal, m_min=10, m_max=50, Xval=Xval, yval=yval, autoscaling=1)
